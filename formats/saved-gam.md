...
| Offset   | Width   | Field              | Meaning                                                                                                       |
|----------|---------|--------------------|---------------------------------------------------------------------------------------------------------------|
| `0x0202` | 2 bytes | Food               | Little-endian word, factory value 63. Not touched by the per-turn clock cleanup: the separate party status/provision pass subtracts the eating-member count from it on an hour change, and only at 06:00, 12:00, and 18:00 (`systems/time.md` Section 5); subtraction floors at zero and ordinary grants cap at 9999. Blackthorn rescue performs a separate one-time safety grant: if the complete word is zero it stores 63, otherwise it preserves the word exactly. This is not a captivity counter. |
...
| `0x020A..0x0219` | 16 bytes | Special / quest items | One-byte counters or flags for carried/useable special items. Confirmed members include Magic Carpet at `0x020A`, skull/special key stock at `0x020B`, Amulet/Crown/Sceptre of Lord British at `0x020D..0x020F`, shard flags at `0x0210..0x0212`, Spyglass at `0x0214`, HMS Cape plans at `0x0215`, Sextant at `0x0216`, Pocket Watch at `0x0217`, Black Badge at `0x0218`, and the Wooden/Sandalwood Box story flag at `0x0219`. The byte at `0x0219` is the save-backed box flag: item acquisition sets it and the endgame reads it. **`0x020C` is not a carried-item counter**: it is the fixed hidden-treasure daily cooldown cookie described in Section 10, and it happens to live in this band. Normal acquisition writes the no-quantity marker `255` into each of the three shard flags, and the U-Use picker lists an entry exactly when its stored value is nonzero, so a shard is present-or-absent rather than counted (`systems/inventory.md` Section 4.5). **Two bytes of this band reach no picker entry at all**: `0x020C` above, and `0x0213`, the byte immediately after the third shard flag, which is the byte an out-of-range shard grant writes (`systems/containers.md` Section 8) and which therefore grants nothing the player can see or use. Other individual meanings remain cross-system. |
...
| `0x02E2`     | Moral-standing selector        | One-byte capped standing/progression value used by shrine rewards, Blackthorn rescue/refuge verdict text, the Lord British camp event, and several action penalties. Blackthorn reads this same byte for its verdict and later raises it to 75 when lower; there is no second rescue-progression byte. This is not the party food word at `0x0202`. The factory seed value is `75`. Preserve adjacent unnamed bytes in this band. |
...
| `0x03A9`     | Door auto-close previous tile | One-byte snapshot of the closed tile replaced by O-Open. Zero means no pending auto-close. Although Journey Onward reads this byte from disk, town location setup clears it before the first playable turn and does not reapply the open tile. |
...
- The combat action-result byte's saved position, vanish-suppression consumer,
  sleep overwrite, and per-dispatch reset are derived from private analysis in
  `u5-decomp/functions/COMBAT_OVL/`, `u5-decomp/functions/COMSUBS_OVL/`,
  `u5-decomp/functions/ULTIMA_EXE/`, and `u5-decomp/notes/`.
...
- The Blackthorn rescue tail's zero-only Food grant and its use of the existing
  moral-standing selector, with no captivity or parallel progression field,
  are derived from private analysis in `u5-decomp/functions/BLCKTHRN_OVL/`,
