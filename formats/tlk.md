...
| `0x88` | ASK-WHO             | No argument byte. Ask the player to name someone and read a typed line. On a match against a member in a roster slot below the current party count — member status is not consulted — set the active scene's branch-flag bit for the NPC currently speaking, which is the in-stream setter that `0x8C` tests, and print the affirmative acknowledgement; on empty input, and again on a completed scan that matched nothing, print the dismissive one. Both acknowledgements are stored engine text, not stream text, and the runner discards the double quote the stream commonly carries immediately after this byte; `systems/conversation.md` Section 7.6 owns the exact output. *(Corrected 2026-09-12, issue #266: the qualifier "live party member" is withdrawn — R486.)* |
