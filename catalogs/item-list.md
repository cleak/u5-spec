...
weight statistic is covered below. A pickup record carries one inventory class byte that both admits it to `G` Get
and selects its grant arm: `systems/containers.md` owns that gate and its
class-to-family map, and `systems/hidden-treasures.md` owns fixed Search
placements, while this catalog owns item identity, stock rows, and item-use
semantics. Equipment ids remain a separate space, carried in the record's
auxiliary byte. R-Ready owns readiness-time ammunition requirements and
...
weapon": the Morning Star and Halberd are in-hand melee weapons with cap 2, and
the Dagger and Spear are throwables. Only the five true missile items - Sling,
Flaming Oil, Bow, Crossbow and Magic Bow - additionally run the adjacent-attacker
interference abort specified in `systems/combat.md` Section 8.2, which cancels the
attempt (while still spending the turn) when an automatic-driver-side actor that
recently struck the attacker is standing in one of the eight surrounding cells. A range cap of zero means the ordinary
...
| Ring of Regeneration | Ring | Ring-slot equipment. After accepted R-Ready, it has a 1-in-16 immediate vanish check. Outside combat, a regeneration check gives each non-Dead wearer a 1-in-8 chance to recover exactly 1 HP, capped at maximum HP. It is reached once per turn-consuming action in world, town, and dungeon modes and once per ten-minute town-bed rest step, in both cases as the last step of the shared party status pass, and once per five-minute wilderness camp step, in that case called directly by the camp loop rather than through the status pass. It is never an hourly effect. In combat, the regeneration tick is applied both by the pass that seats the party at combat entry and again by the combat parser's committed-action tail, so a wearer can take more than one tick per fight; a free refusal branches back before that tail. The 1-in-16 vanish check is encounter-entry-only: one independent check in that same seating pass, taken **before** the member is placed and before that member's own ring-effect step, destroying the ring on a single one of the sixteen outcomes; it is not repeated by committed actions and not by the combat round loop. **Corrected (issue #178, 2026-08-31):** the combat-entry regeneration tick is a **whole-party sweep**, not one tick for the wearer that triggered it - it runs only for a member whose status is exactly Good or Poisoned, and then draws one uniform `0..7` for every party member alive and wearing the ring at that moment, so two eligible wearers produce two sweeps. Any closed-form draw count for seating is withdrawn; see `systems/combat.md` Section 5.3. Two earlier readings are withdrawn: that the in-combat removal is performed by the round loop, and that worn-ring effects run only at seating so a wearer gets exactly one regeneration tick per fight (`RETRACTIONS.md` R266, R267; contract in `systems/combat.md` Sections 12 and 14). | Non-combat cadence lives in `systems/time.md`; combat healing cadence lives in `systems/combat.md`. |
...
sets the plans flag instead of a scroll counter. The **Code** column below is the shipped two- or three-character rune label as the inventory and pickup narration print it (`systems/inventory.md` Section 4.5 and `systems/containers.md` Section 6 give the same eight); it is not the letter code the spell tables use for the related spell, which is permuted for five of the eight rows.

| Scroll id | Code | Cross-reference | U-Use effect and gates |
|-----------|------|-----------------|------------------------|
| 0 | `VL` | Vas Lor / Great Light | Prints `Light!`, sets the magic-light counter to 240 minutes, and plays the light presentation effect. It writes the light counter, not the timed-effect slot, and it is not a torch inventory consumer. |
| 1 | `RH` | Rel Hur / Wind Change | Prints `Wind change!`, prompts for a wind direction, and applies the wind only when the active scene byte is below 33: overworld or named town/castle/keep/dwelling scenes. Dungeon-class scenes return without changing wind after the prompt. |
...
| 3 | `IA` | In An / Negate Magic | Prints `Negate magic!` and installs the shared timed-effect tag `N` with a 20-turn duration, replacing whatever effect the slot held. The party C-Cast absorption path and the three enemy-side class-special, teleport, and scene-resistant ranged/effect checks read the same tag as the cast spell path; `systems/magic.md` owns their distinct action-consumption rules. |
...
| 5 | `KXC` | Kal Xen Corp / Summon | Prints `Summon Daemon!`. Combat-class scenes invoke the summon **placement** helper; non-combat scenes print `Not here!` and perform no summon. *Corrected 2026-08-23: an earlier revision called it the "summon/target animation helper". That is withdrawn - the helper takes no target (it overwrites any cached coordinate with fresh random draws on its first probe), and its animation step is incidental to what it does, which is create a hostile actor at the first accepted cell. `catalogs/spell-list.md` owns the full contract.* Combat `U` does reach the item-use handler (an earlier note that it did not is withdrawn), so this effect is reachable both from the scroll handler and from the combat command's item-use path where the family's own gates allow. |
| 6 | `IMC` | In Mani Corp / Resurrect | Prints `Resurrection!`. Non-combat scenes prompt for a party member, invoke the same resurrection helper described in `systems/magic.md`, and refresh the party stats display. A successful scroll resurrection leaves the revived member Good with 1 current HP, class-derived mana, recomputed level, and maximum HP equal to thirty times that level. Combat-class scenes print `Not here!`. |
...
|------|------------|------|
| Moonstone | Story item tied to Blackthorn rescue/refuge handling (`systems/blackthorn.md`) and Gate Travel. Burying one records the current valid location into that stone's saved gate slot when the scene byte is `0x00..0x20` and the underfoot tile is `4..10`, `44`, or `45`; the whole transcript is the label `Moonstone_` followed by `buried!` or by the single all-purpose refusal `cannot be buried here!`, with no transport test and no `Failed!` tail. *Vas Rel Por* later teleports to the selected slot. Recovery is two directional commands on the cell **next to** the party: Search into the buried cell prints `a strange rock!` and stages the stone without granting it, and a following Get into that cell prints `A moonstone!` and grants it, silently invalidating the slot. | This catalog owns the carried item and use/recovery contract. The rescue/refuge presentation is owned by `systems/blackthorn.md`; the saved destination slots are owned by `formats/saved-gam.md`. |
| Grapple | Quest mobility item/flag. Lord Michael's conversation grant sets the Grapple flag used by outdoor K-Klimb. On the overworld and underworld planes, K-Klimb refuses without this flag - and refuses before it ever asks for a direction. With the flag, on foot, and with a *prompted* direction naming the single climbable tile identity (there is no facing probe; `RETRACTIONS.md` R470), each living party member rolls Dexterity against `1..30` and may take `1..5` fall damage before the party advances one cell. | Exact inventory display label/stock presentation, if any. |
...
| Sandalwood Box | Story item returned to Lord British in the endgame sequence. Saduj's hostile clue identifies Lord British's chamber as the story route for discovering the hidden box's importance. The shipped castle roster places the non-speaking `CASTLE:0` object slot 31 at local `(18,12,2)` carrying inventory class `0x0E`; `G` Get accepts the record because that class is below sixteen, and the **same** byte selects the Sandalwood Box arm of the shared Search/Get/container inventory-add writer, which sets the save-backed box flag. The pickup prints `A sandalwood box!`. The endgame confirmation reads that flag. | No traced acquisition handler requires Saduj's conversation as a mechanical prerequisite; his branch is a clue. *(Corrected 2026-09-12, issue #262: the acceptance test and the grant selector are one byte, not a visual filter plus a separate code - `RETRACTIONS.md` R476.)* |
...
| Shard of Falsehood | Shadowlord shard story item. Normal acquisition writes the no-quantity marker `255` into its carried flag, so its picker row is uncounted (`systems/inventory.md` Section 4.5). U-Use dispatches shard index 0 into the Shadowlord-destruction handler. A successful destruction consumes the shard: the handler clears its carried flag together with retiring the Shadowlord. A refused attempt keeps it. | Search/Get/container grants use the same shard index; destruction requires the matching interior destruction position and active Faulinei encounter, as specified by the quest graph. |
| Shard of Hatred | Shadowlord shard story item. Normal acquisition writes the no-quantity marker `255` into its carried flag, so its picker row is uncounted (`systems/inventory.md` Section 4.5). U-Use dispatches shard index 1 into the Shadowlord-destruction handler. A successful destruction consumes the shard: the handler clears its carried flag together with retiring the Shadowlord. A refused attempt keeps it. | Search/Get/container grants use the same shard index; destruction requires the matching interior destruction position and active Astaroth encounter, as specified by the quest graph. |
| Shard of Cowardice | Shadowlord shard story item. Normal acquisition writes the no-quantity marker `255` into its carried flag, so its picker row is uncounted (`systems/inventory.md` Section 4.5). U-Use dispatches shard index 2 into the Shadowlord-destruction handler. A successful destruction consumes the shard: the handler clears its carried flag together with retiring the Shadowlord. A refused attempt keeps it. | Search/Get/container grants use the same shard index; destruction requires the matching interior destruction position and active Nosfentor encounter, as specified by the quest graph. |
...
| Skull Keys | Named key-family stock. U-Use spends one key, then asks `Direction-` and tries the selected lock in town/overworld or combat. Dungeon exploration prints `Not here!` after spending the key. The former combat refusal is withdrawn (R415); ordinary `J` Jimmy remains separate. | This row owns carried stock identity. `systems/inventory.md` Section 7.1 specifies the exact prompt, target changes and failure result. |
...
U-Use and set the corresponding ownership flag to the no-quantity marker `255`.

**The grant masks an out-of-range sub-index to two bits rather than rejecting
it.** The behaviour is periodic with period four across every value a sub-index
byte can carry: values congruent to 0, 1 and 2 set the Falsehood, Hatred or
Cowardice flag respectively and print that shard's own text, so a sub-index of
four grants a real Shard of Falsehood, while values congruent to 3 write the
unused byte after the third shard flag (`formats/saved-gam.md` Section 7),
printing the Cowardice text while granting nothing that can ever appear as a
picker row. The flag written and the line printed are driven by the same masked
index, so the two never disagree. A compatible implementation should either
reject an out-of-range shard sub-index outright or reproduce the two-bit mask;
it must **not** model out-of-range as a blanket no-op, because three of every
four out-of-range values grant a real shard. Whether any shipped content ever
passes a sub-index above 2 is not established - the grant's callers were not
enumerated (`OPEN-QUESTIONS.md`).
...
The byte that makes `G` Get legal **is** the byte that selects the item
awarded: one inventory class byte on the object record, accepted when it is
below sixteen or is one of the four special classes, then handed straight to the
grant dispatcher. The Sandalwood Box pickup is the worked example - its class is
below sixteen, and that class is the Box arm. *(Corrected 2026-09-12, issue
#262: this paragraph previously said the two were "not necessarily the same
byte" and asked implementations to preserve that separation - `RETRACTIONS.md`
R476.)* Equipment ids remain a separate space, carried in the record's auxiliary
byte.
...
Sextant turn-accounting closure (2026-08-23): every returning non-combat U-Use
path is classified as one normal action by the command layer, independently of
whether the selected item succeeds, refuses, or is never selected. The
successful reading, daytime refusal, wrong-place refusal, picker cancellation,
and no-usable-item refusal therefore all run the current exploration mode's
ordinary per-turn processing. Source provenance: derived from private analysis
in `u5-decomp/functions/CAST_OVL/`, `u5-decomp/functions/ULTIMA_EXE/`,
