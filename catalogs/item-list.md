...
| 3 | `AI` | In An / Negate Magic | Prints `Negate magic!` and installs the shared timed-effect tag `N` with a 20-turn duration, replacing whatever effect the slot held. The party C-Cast absorption path and the three enemy-side class-special, teleport, and scene-resistant ranged/effect checks read the same tag as the cast spell path; `systems/magic.md` owns their distinct action-consumption rules. |
