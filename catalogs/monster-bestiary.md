...
- **Reward unit** - the small raw value returned by the damage/death handler
  ...
  `floor(HP / 4) + 1`. This catalog does not define the final XP, gold, karma,
  or score effect of that value.
- **Drop cap** - the raw bound used by the default monster-death drop roll. Zero
  means the current notes do not show a default loot-quantity bound for that
  class; it does not prove the absence of all post-kill effects.
...
Armor, attack damage, spell power, movement cadence, unnamed flag bits, and the
AI script interface are not fully decoded yet; section 8 records those gaps.

## 2. Shared AI And Reward Units

Monster turns run through the same command-dispatch architecture as player
turns: an AI path produces an intent and direction, synthesizes the equivalent
command byte, and then uses the normal combat command parser. The target picker
scans the 32 combat slots, filters out empty, dead, hidden, and same-faction
actors, then chooses the closest surviving enemy. A flee flag in the actor record
reverses the chosen direction, so the same movement code handles both pursuit
and retreat.

Per-class AI scripts exist, but the script-call ABI, state-block meaning, and
bytecode format have not been decoded. The bestiary therefore records only the
class traits confirmed by the damage and target-picker readers:
...
| `vanishes` | On death, the class clears through a vanish path instead of the default corpse/loot path. |
| `special death` | A class-specific tile/effect transition runs on death. |

Default monster kills can update the active-object table with post-kill markers.
The current notes confirm only the raw quantity marker and special-drop marker;
the final item, gold, XP, karma, or score interpretation is still open. Classes
that vanish or use a special death path may bypass the default drop path.
...
| Class | Creature | Sprite run | HP | Reward unit | Drop cap | Traits | Encounter context |
|------:|----------|------------|---:|------------:|---------:|--------|-------------------|
| 16 | Sea Horse | `0x80..0x83` | 30 | 8 | 0 | - | Water encounters |
| 17 | Squid | `0x84..0x87` | 50 | 13 | 0 | - | Water encounters |
| 18 | Sea Serpent | `0x88..0x8B` | 70 | 18 | 0 | - | Dangerous water encounters |
| 19 | Shark | `0x8C..0x8F` | 22 | 6 | 0 | - | Water encounters |
...
floating, terrain immunity, or specialized AI script selection.
...
| Class | Creature | Sprite run | HP | Reward unit | Drop cap | Traits | Encounter context |
|------:|----------|------------|---:|------------:|---------:|--------|-------------------|
| 20 | Giant Rat | `0x90..0x93` | 10 | 3 | 5 | - | Low-tier wilderness or trap encounter |
| 21 | Bat | `0x94..0x97` | 5 | 2 | 0 | - | Low-tier dungeon or night encounter |
| 22 | Giant Spider | `0x98..0x9B` | 10 | 3 | 5 | - | Wilderness, dungeon, poison-themed encounter |
| 23 | Ghost | `0x9C..0x9F` | 20 | 6 | 0 | physical half | Undead or spectral encounter |
| 24 | Slime | `0xA0..0xA3` | 10 | 3 | 0 | splits | Replicating dungeon creature |
| 25 | Gremlin | `0xA4..0xA7` | 10 | 3 | 12 | - | Dungeon, trap, or nuisance encounter |
| 31 | Insect Swarm | `0xBC..0xBF` | 5 | 2 | 0 | - | Swarm encounter; also associated with the Insect Swarm spell |
| 34 | Python | `0xC8..0xCB` | 10 | 3 | 0 | - | Snake or trap encounter |
| 46 | Rot Worm | `0xF8..0xFB` | 5 | 2 | 0 | - | Dungeon or underworld vermin encounter |
...
| Class | Creature | Sprite run | HP | Reward unit | Drop cap | Traits | Encounter context |
|------:|----------|------------|---:|------------:|---------:|--------|-------------------|
| 26 | Mimic | `0xA8..0xAB` | 30 | 8 | 20 | team override | Chest-like or ambush-style monster |
| 27 | Reaper | `0xAC..0xAF` | 40 | 11 | 25 | team override | Forest or fixed dungeon encounter |
| 28 | Gazer | `0xB0..0xB3` | 20 | 6 | 0 | special death | Eye-burst death effect |
| 29 | Crawler | `0xB4..0xB7` | 35 | 9 | 0 | - | Dungeon or underworld encounter |
| 30 | Gargoyle | `0xB8..0xBB` | 40 | 11 | 0 | splits; team override; special death | Terrain-hazard transition before normal cleanup; exact visual needs verification |
| 32 | Orc | `0xC0..0xC3` | 10 | 3 | 11 | team override | Humanoid wilderness or dungeon group |
| 33 | Skeleton | `0xC4..0xC7` | 20 | 6 | 13 | physical half | Undead encounter |
| 35 | Ettin | `0xCC..0xCF` | 30 | 8 | 17 | team override | Large humanoid encounter |
| 36 | Headless | `0xD0..0xD3` | 20 | 6 | 12 | team override | Wilderness or underworld encounter |
| 37 | Wisp | `0xD4..0xD7` | 40 | 11 | 0 | - | Magical or spectral encounter |
| 38 | Daemon | `0xD8..0xDB` | 75 | 19 | 0 | physical half | High-tier magical/dungeon encounter |
| 39 | Dragon | `0xDC..0xDF` | 99 | 25 | 30 | - | High-tier wilderness or dungeon encounter |
| 40 | Sand Trap | `0xE0..0xE3` | 80 | 21 | 25 | - | Desert or fixed trap-like encounter |
| 41 | Troll | `0xE4..0xE7` | 15 | 4 | 15 | - | Mountain, bridge, or wilderness encounter |
| 44 | Mongbat | `0xF0..0xF3` | 20 | 6 | 5 | - | Flying or cave-style encounter |
| 45 | Corpser | `0xF4..0xF7` | 40 | 11 | 0 | - | Underworld or fixed dungeon encounter |
...
| Class | Actor | Sprite run | HP | Reward unit | Drop cap | Traits | Encounter context |
|------:|-------|------------|---:|------------:|---------:|--------|-------------------|
| 12 | Guard | `0x70..0x73` | 99 | 25 | 5 | - | Town hostility or scripted guard fight |
| 13 | Wanderer | `0x74..0x77` | 99 | 25 | 0 | physical immune; vanishes | Special NPC combat class |
| 14 | Blackthorn | `0x78..0x7B` | 99 | 25 | 0 | physical immune; vanishes | Scripted boss or special encounter |
| 15 | Lord British | `0x7C..0x7F` | 99 | 25 | 0 | physical immune; vanishes | Special protected NPC class |
| 47 | Shadow Lord | `0xFC..0xFF` | 99 | 25 | 0 | physical half; vanishes; special death | Scripted Shadowlord encounter |
...
- Initial HP, raw reward unit, raw drop-cap byte, active-object sprite run, and
  decoded combat traits for every listed class.
...
2. The AI script ABI and bytecode runner are not decoded, so per-class tactics
   beyond closest-target pursuit, flee inversion, and faction overrides are
   incomplete.
3. The exact terrain-to-monster and dungeon-room-to-monster distribution tables
   ...
   spawn tables.
4. Drop contents and reward consumers are not decoded. The default kill path can
   set a raw quantity marker and a special-drop marker, and the damage/death
   handler returns a raw reward unit, but the final item, gold, XP, karma, or
   score interpretation is still open.
...
- COMSUBS overlay AI direction/script-dispatch note - per-class AI script
  dispatch and unresolved script ABI.
