...
| Item | Where | Notes |
|---|---|---|
| Issue #238 confirms Return selects a Sell row but still reports no arrow movement. | `systems/shops.md` Section 8.1 | Capture Down or Right with a following nonempty row, supplying the starting save and asset identity, exact key events, and before/after frames of the highlighted row and conversation window. Isolated dispatch and command-normalization checks confirm accepted commands but do not establish delivery through the live keyboard stack. |
