...
more trace items (section 3) and two more capture items (section 2). What else
...
| Item | Where | Notes |
|---|---|---|
| Issue #259 reports variable zero/one/two-row U completion gaps. Original picker-panel overflow plus EGA's fixed strip scroll reproduces this mechanism with an unchanged message cursor; the exact reported run is not replayed. | `systems/inventory.md` Section 7.1 | Obtain stock quantities and complete original picker/message frames immediately after U, after each navigation key and through acceptance, or a standalone original-game save with exact inputs. Correlate each displaced row with a panel overflow; symbolic-glyph spatial probes do not establish the reporter's exact inventory or raster. *Narrowed 2026-09-12 (issue #264): the counted-row wrap is now measured in the original's own column and row bookkeeping (`systems/inventory.md` Section 4.5), so the overflow mechanism no longer rests on inference; the reported run is still unreplayed.* |
| The conversation renderer's column-dependent flush rules - a leading space dropped at column zero and a row break swallowed at column fifteen or beyond - were read but only partly exercised, because the emulation harness has no real screen state. The entry sequence, the quote-suppression rule and the column-eighteen word break are established. | `systems/conversation.md` Sections 6 and 9 | A capture with real cursor state through an NPC entry and a long wrapped response. |
...
| What the display driver paints from a wrapped counted picker row. The wrap itself - a label moved whole to the next display line, with the renderer's padding landing there - is established in the original's own column and row bookkeeping; no raster, font or pixel result is claimed, and the second line's cells are emitted past the frame's right rule in one case. | `systems/inventory.md` Section 4.5; `systems/text-output.md` Section 6 | A raster capture of a U-Use picker holding a counted shard row, reconciled with the published glyph contract rather than asserted from the character stream. |
...
| How many items a picker page holds once a counted row wraps, given that such a row occupies two display lines rather than one. How the page-full test counts those lines was not established, and only single-page captures were taken, with the picker exited at the first key read. | `systems/inventory.md` Sections 4.4 and 4.5 | Enumerate a full page with counted shard rows present and record where the page break falls. |
