...
| Item | Where | Notes |
|---|---|---|
| Issue #259 reports the U family completion two rows below Item at column six; original-code execution retains the prompt cursor and has no leading prefix. | `systems/inventory.md` Section 7.1 | Obtain a stock save, canonical asset identity, exact inputs and full original frames before U, at picker wait, immediately after acceptance and through the next command wait, with message rows identified. Current probes observe text/cursor calls, not final raster contents. |
