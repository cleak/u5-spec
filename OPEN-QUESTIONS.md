...
items (sections 3 and 2). What else
...
| Which side of the slow-CPU threshold a real install's boot calibration word lands on during gameplay, and whether it can land in the high half of the range at all. The gate itself is settled and signed (R491), and only the intro's own installed override is known to sit above the baseline. Every presentation pause in the program that requests a single BIOS tick depends on this. *Opened 2026-09-12 (issue #268).* | `systems/timing.md` Sections 4 and 5.2; `systems/blackthorn.md` Section 4.2 | Read back the calibration word at a gameplay prompt on a slow host, a period-typical host and a fast emulated host. |
