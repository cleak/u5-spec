...
finds only outdoor chunk-loader coordinate reuse, town asterisk-marker reuse,
...
**Current combat post-round maintenance cleanup:** 2026-05-12 -
`systems/combat.md` and extraction now promote the resident combat post-round
pass: it sweeps the eleven-by-eleven runtime arena terrain/effect grid,
dispatches normal, cooldown-sensitive, and table-mapped cell effects, blinks
the player cursor on alternating passes, and optionally draws a secondary arena
marker. This pass is presentation/effect synchronization; it does not own
placed-field lifetime, actor turn dispatch, or combat time advancement.
...
chunk-loader scroll-position reuse, town-entry asterisk-marker reuse, and
...
**Current corrections propagation:** 2026-05-12 - All `u5-decomp/CORRECTIONS.md` headings have been reconciled into clean public prose, the new PRNG, timing, moon-display, and runtime specs are tracked in `EXTRACTION.md`, and exact-parity follow-up remains concentrated in the declared partial rows rather than the closed corrections backlog.
...
**Current remaining checklist audit:** 2026-05-12 - after the ULTIMA checklist pass and SHOPPES2 tavern-provision reclassification, the only remaining unchecked `Spec drafted` box should be the function-note template. `SHOPPES2_OVL/0x0450_food_pay_and_serve.md` is now resolved as a tavern/meal-counter provision helper with clean prose in `systems/shops.md`.

**Current SHOPPES2 private-note correction:** 2026-05-12 - `SHOPPES2_OVL/_OVERVIEW.md`, `TALK_OVL/0x00E6_shop_dispatch.md`, `ULTIMA_EXE/0x75CC_overlay_loader.md`, and `SHOPPES2_OVL/0x0450_food_pay_and_serve.md` now align with the clean shop contract: Talk trigger `0x84` enters the SHOPPES2 shipwright flow, not a food/provisions merchant, while the internal `0x0450` block belongs to the tavern/meal-counter provision branch.
