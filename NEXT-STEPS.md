...
**Current combat spell-prereq cleanup:** 2026-05-07 - combat, magic, and DATA.OVL docs now name the per-slot combat cast target mapping, narrow the pre-gate failure/continuation semantics, and leave the resource/allowed check body plus target-reaction hook as open.
...
**Current combat AI staging cleanup:** 2026-05-07 - combat and monster-bestiary docs now distinguish the monster intent selector from the later class-script direction runner, keeping the intent class-effect map and runner instruction/effect map open.
...
**Current non-light countdown cadence cleanup:** 2026-05-07 - magic, spell-list, and extraction docs now separate P/Q/C/N helper-invocation aging from the combat time/render cleanup: the latter runs on ten ready-action wraps, while non-light counter exactness still depends on the post-action/no-target fallback helper body and Time Stop decrement path.
...
**Current monster-AI cleanup:** 2026-05-07 - public combat/magic/DATA.OVL wording now identifies the class-script dispatch boundary, live-state versus inactive-script split, and still-open runner instruction set/class-effect map without exposing private offsets or implementation text.
...
**Current combat target-picker cleanup:** 2026-05-07 - combat, magic, and monster-bestiary docs now publish the target-picker's separate phase/hidden suppression filter, ordinary invisibility filter, first-five-party-slot fallback guard, centre fallback, pending-action marker behavior, and monster-spell separation from the party C-Cast dispatcher while keeping the COMSUBS AI runner instruction set and class-to-effect map open.

**Current combat fleeing cleanup:** 2026-05-07 - combat, spell-list, monster-bestiary, and extraction docs now reconcile Cause Fear as the confirmed public fleeing-flag writer while narrowing the remaining flee gap to non-Cause-Fear writers such as morale, class-script decisions, or monster special effects.

**Current combat command-dispatch cleanup:** 2026-05-07 - combat and extraction docs now clarify that the dispatcher-level combat command map is complete for all twenty-six letters plus seven special inputs; remaining command work is the delegated branch-body semantics, not recognition coverage.

**Current monster AI state cleanup:** 2026-05-07 - combat, magic, DATA.OVL, spell-list, monster-bestiary, and extraction docs now publish the class-scoped AI storage contract: live actors of the same class share mutable class state, dead/inactive actors use the class's static script entry, slot-local facts stay in the combat actor/effect tables, and the remaining AI gap is state-field/runner/effect semantics rather than storage ownership.
...
- 62 cleanroom spec docs: 30 system specs, 25 format specs, and 7 catalogs.
...
- Recent additions: `systems/vehicles.md`, `systems/containers.md`, `systems/commands.md`, `systems/launcher.md`, `systems/animation.md`, `systems/display-driver.md`, `formats/font-ch.md`, `formats/font-hcs.md`, `formats/font-pcs.md`, `formats/bit.md`, `formats/look2-dat.md`, `formats/signs-dat.md`, `formats/question-dat.md`, `formats/karma-dat.md`, `formats/story-dat.md`, `formats/endmsg-dat.md`, `formats/miscmsg-dat.md`, `formats/shoppe-dat.md`, `formats/end-dat.md`, `catalogs/gazetteer.md`, and `catalogs/quest-graph.md`.
- Local verification expanded all 55 LZW resources in the analyzed asset set
  (`.16`, `.4`, `PROPORT.PCS`, `TITLE.BIT`, and `BRITISH.BIT`) to their
  declared decoded lengths with zero mismatches.

### Remaining high-value gaps

- Non-optional Priority A prose docs are now covered for the analyzed DOS baseline.
- The shared LZW codec for `PROPORT.PCS`, `TITLE.BIT`, and `BRITISH.BIT` is now specified and locally verified against the analyzed asset set. Fixed title bitmap placement, `BRITISH.PTH` pen origins, intro title/menu idle ticking, intro story-step transition/draw mapping, intro story rectangle-transition region/order and helper handoff, Return-to-View `MISCMAPS.DAT` stream ownership, command table, helper-effect schedule, and the public display/rendering contract are now specified. Remaining exact visual parity gaps are the story rectangle-transition resident helper wipe curve, Return-to-View resident helper raster/pacing internals, exact binary `EGA.DRV` behaviour if hardware parity becomes in scope, and the already-called-out per-table semantic enumerations. `.XMI` music is not present in the analyzed clean DOS baseline; add it only for a different audio-enabled distribution.
- The spell table is now aligned to the resident incantation order (`In Lor`, `Grav Por`, ...), with all 48 parser tokens, parser acceptance rules, recipe masks, scene masks, light-spell durations, the 99-charge mix cap, major CAST handler families, active-target attack-wrapper damage math for Magic Missile/Fireball/Kill, Tremor exact damage/reward and no-faction-filter actor scan, shared active-effect tag/counter values and expiry behavior, Protection's active-effect defense bonus, Quickness's player-dispatch random gate, Mass Charm's class-threshold target-selection remap, Negate Magic's combat-cast absorption path, Charm/Polymorph/Clone/Fear/Gate/Time Stop high-circle handler semantics, Polymorph's Giant Rat replacement, Clone's paired-slot allocation, random legal arena placement, no-partial-copy capacity failure, undefined original capacity result, shrine-meditation linkage to Avatar intelligence, dungeon field no-write failure, the combat post-step boundary for field/hazard contact, the arena-field helper placement/application split, combat field-kind bytes, coordinate-lookup slot/flag eligibility, active-object marker contact for placed combat fields, non-consuming field contact, accepted-placement redraw/lifetime non-ownership, combat field lifetime until combat exit, no friend/foe gate in the shared field-contact scan, Poison/Sleep field-contact status gates, Fire Field raw 1..21 damage before defense, Energy Field raw zero damage/value input, and directed In Zu/Poison Wind/Death Wind/Flame Wind target-walk friendly-fire behavior documented semantically. Remaining magic/combat parity gaps are the monster combat-AI runner instruction set/class effect map and exact non-light countdown decrement cadence, not the forty-eight-entry player spell table.
...
- Analyze broader EGA.DRV behaviour only if exact display-driver behaviour becomes a required public spec.
- If magic/combat parity becomes the next target, continue from the COMSUBS AI script runner and monster-class effect branches to finish the monster combat-AI runner instruction set and class effect map.
