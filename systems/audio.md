...
| Monster possession success; the controlled-party faint/release pass, wherever it is reached | These triggers use the identical software envelope: `(delta 2, initial comparison 1000, 30000 iterations, idle 1, period 3100)`. Possession runs it after success narration; resistance skips it. The faint path prints `<party member> passes out!`, runs this envelope, and only then removes the Sword of Chaos and applies sleep. The faint path is **not** exclusive to a vanish-on-death kill: the same pass, with the same envelope, is also reached from a Shadow Lord's self-acting arena escape, from the AI target picker's no-candidate fallback and from the round loop's defeat arm (`systems/combat.md` Sections 6.3, 9.1 and 14). Naming only the vanish caller was an incomplete caller list, not a different contract. |
