...
The write wrapper is a three-argument routine: byte count, source buffer, and filename. Like the read wrapper, it provides retry-with-prompt. It also momentarily swaps the resident critical-error handler to a write-specific variant for the duration of the write, restoring the read/disk-prompt variant afterward. This write-specific handler belongs to the same resident disk-error dispatch cell documented in `systems/disk-prompt.md`. It is the write-side error-reporting path used while a create/write operation is in progress, and its visible effect is a "your disk may be write-protected, try again" message followed by a keypress wait. (Earlier revisions of this paragraph attributed the cell to a "screen-mode dispatch" controller; that framing is withdrawn.) The exact historical wording of the write-error prompt remains a floppy UI detail. The wrapper retries while the inner writer reports zero bytes written.

**The wrapper first requires the target file to exist.** Before it calls the inner primitive at all, it selects the save disk and then repeatedly tries to *open the named file as an existing file*, going through the disk-prompt handler on every failure, until the open succeeds. Only then does it create-or-truncate and write. The engine therefore never creates `SAVED.GAM` or `SAVED.OOL` from nothing: a missing file is treated as "the wrong disk is in the drive" and the write blocks in that loop. In character creation this is visible as a hang on a blank screen after the closing narrative if either file is absent; the shipped distribution carries both, which is why it is never seen in ordinary play. *(Added 2026-09-05; observed live and read from the wrapper.)*
