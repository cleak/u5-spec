...
1. **Input.** Block on the input pipeline until a keystroke arrives. The pipeline polls the keyboard, paints the cursor blink, and — when no key is pending — runs one *world tick* of redraw and ambient animator-driven side effects. In-world time and scheduled NPCs freeze during the wait; the idle tick is not a committed turn. A pending stats-panel refresh request is drained at the head of a command prompt, never at a turn boundary, and the idle tick itself repaints no panel. For the overworld and town that drain sits in this shared input pipeline, after its world-tick call and before the prompt newline and the key read — but only on the pipeline's full-prompt arm, not its quick-poll arm. The dungeon and combat loops drain at their own prompts instead, at the points `systems/stats-panel.md` Section 2.3 gives; that section owns all four (see also Section 2.2).
...
Every mode loop's per-turn epilogue calls a single shared helper — the *per-turn cleanup* — with a minute increment. The helper does five things, in order, and a sixth on a day boundary:
...
5. **Hour-change presentation refresh.** If the hour changed, the active scene uses the surface/town status presentation, **and** the party's Z value has its high bit clear, refresh the sky strip in the top viewport border (`systems/moons.md`). Those are two separate tests and both must pass (`systems/time.md` Section 5 item 2): the Z test keeps an hour crossing from repainting in the Underworld or on a below-entry floor, which is the one thing that keeps this caller off the renderer's erase arm — `systems/moons.md` Section 3 lists the other callers, which do reach it. Earlier notes treated this as an unresolved gameplay event hook; the resolved public contract is narrower and display-only. Natural-moongate placement is still separate open overworld work.

6. **Day-rollover stats-panel repaint.** When, and only when, the hour wrapped and the day advanced, repaint the whole stats panel. A zero increment, a pass that leaves the minutes short of an hour, and one that leaves the hour short of a day all bypass it; the day-in-range, month-roll and year-roll arms all reach it. It therefore fires exactly once per in-game day boundary, and it is the only time-driven panel refresh in the game — a backstop only where and when this helper is actually called (`systems/stats-panel.md` Section 2.4, `systems/time.md` Section 7).
...
After producing the scratch grid, the tick refines visibility by toggling certain marker tiles through the shared squared-distance lookup centred on the viewport, then renders. Its tail then calls the ambient-audio tick whenever the shared first-tick hint is non-zero — which is almost every tick — and normalises that hint afterwards. **The world tick repaints no stats panel, on the first tick after a mode entry or on any other**, and neither does the per-frame active-object animator; no route from either reaches the full-panel refresh in any call form (`systems/stats-panel.md` Section 2.2).

> **Corrected 2026-09-12 (R488).** This paragraph previously ended "On the very first tick after a mode entry, an additional full-panel repaint runs, painting the side panels, status bars, and frame borders that the in-loop renderer otherwise leaves alone." That sentence is withdrawn: the first-tick hint gates the ambient-audio tick, and the side panels are painted only by the refresh's own callers. `systems/input.md` Section 12 carried the same reading and is corrected with it.
