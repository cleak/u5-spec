...
- **Town / dungeon → overworld.** Walking off a town-family boundary tile, or reaching a dungeon scene-clear path, clears the scene byte. Town exits and the dungeon surface-reset helper restore a defined exterior coordinate before the next outer iteration runs the overworld loop. The pit-chain off-bottom path is traced as a dungeon scene-clear that leaves the incremented off-bottom level byte and the trap-chain X/Y in resident state; it does not call the surface-reset coordinate table.
...
5. **Hour-change presentation refresh.** If the hour changed and the active scene uses the surface/town status presentation, refresh the lower sky/status row. Earlier notes treated this as an unresolved gameplay event hook; the resolved public contract is narrower and display-only. Natural-moongate placement is still separate open overworld work.
...
- **2D scene with the visibility-dirty flag set.** Run the centre-out visibility producer against the active map, light radius from the daylight cleanup, local-light mask state, and viewport origin from the player position. Initialise the scratch grid to the "unknown / dark" sentinel, then expand outward from the player and reveal cells as the visibility carve permits. Clear the dirty flag.
