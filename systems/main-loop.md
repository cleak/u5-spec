...
**Layer one — runtime startup.** A short stub at the entry point relocates the stack, zeros the general-purpose registers, and far-returns into the C `main` function. The stub is the canonical preamble produced by the period's compiler toolchain; it does no game-relevant work.
...
Every mode loop reads the scene byte each iteration and exits when the value changes. Every transition between modes is a write to the scene byte — by an entry handler in the source mode (Enter on a town tile, Klimb on a ladder, fall on a chasm, accept on a moongate prompt) or by a combat framer. Mode entry is declarative rather than imperative: the source mode sets the scene byte and returns, and the outer loop dispatches on the next iteration.

Sub-ranges carry additional meaning. In the town family, the high two bits select the class and the low three bits index the eight per-class members; per-class data is addressed by the same encoding. In the dungeon range, the byte's offset from `33` gives a one-based index into the eight named dungeons. The combat range carries an arena identifier. Each mode's spec documents its own range; the outer loop only cares about the four-way split.
...
- **Overworld → town.** Stepping onto an enclosed cell sets the scene byte to a value in `1..32`. The overworld loop's exit check sees the change and returns. The next outer iteration enters the town entry path.
- **Overworld → dungeon.** Klimb on a dungeon entrance sets the scene byte to a value in `33..127` and the overworld loop returns; the next iteration runs the dungeon dispatch.
...
The outer loop reaches those overlays through a static table of *trampoline stubs* that the linker built at link time. There are 164 such stubs in the resident image, each twelve bytes long; each one corresponds to one externally-callable entry point in one overlay. A stub's twelve bytes are: a far-call to the overlay loader, a two-byte overlay index, and a far-jump whose target address is patched at load time when the overlay is brought into memory.

A caller invokes a stub with a plain near-call. The stub's first instruction — the far-call — jumps to the overlay loader. The loader reads the index, advances the saved return address past the index so that on return control will land on the far-jump, and ensures the overlay is present in memory. If the overlay's "loaded" flag is set, the loader returns immediately. Otherwise it reads the overlay file from disk into the appropriate buffer, fixes up the segment field of every stub that references that overlay, and sets the loaded flag. Either way the loader returns; control resumes on the stub's far-jump, which transfers to the overlay's entry function.

From the caller's perspective the stub is a regular function; the callee returns normally. Neither side sees the loader; it is invisibly interposed by the stub.
...
Cross-overlay calls — overlay A calls a stub for an entry in overlay B — work the same way as resident-to-overlay calls. The mechanism is invisible in the call graph.

## 6. The four mode loops

Each world mode has its own per-turn loop. The shared structure is a five-step cycle:

1. **Input.** Block on the input pipeline until a keystroke arrives. The pipeline polls the keyboard, paints the cursor blink, and — when no key is pending — runs one *world tick* of redraw and animator-driven side effects. Time freezes during the wait; the world tick is a pure-redraw cycle.
2. **Pre-dispatch checks.** A short prologue handles meta-state — combat in progress, cursed-by-spell timer — and the scene-byte exit check. If the scene byte changed during the previous turn, the loop returns to the outer loop.
...
4. **Per-turn epilogue.** When the action consumed a turn, the loop calls the per-turn cleanup helper with the mode's minute increment — two minutes for overworld, one minute elsewhere. Mode-specific work runs after cleanup: town runs the NPC schedule processor; overworld runs the active-object animator and the random-encounter check; dungeon runs the torch burn-down; combat advances the round counter.
...
1. **Vehicle adjustment.** If the player is in a skiff, halve the increment with a one-minute floor. If the player is in a tower-style zero-time vehicle, force the increment to zero. The cleanup still runs in the zero case to refresh derived state.
...
The cleanup does not run the NPC schedule processor or the active-object animator. Those are mode-specific subsystems that the mode loop runs after cleanup returns. The cleanup is the time clock; the mode loop is the world clock.
...
After producing the scratch grid, the tick refines visibility (downgrading certain marker tiles past a five-cell radius), then renders. On the very first tick after a mode entry, an additional full-panel repaint runs, painting the side panels, status bars, and frame borders that the in-loop renderer otherwise leaves alone.

The world tick also runs three secondary subsystems before producing the grid: an active-object animator, a small RNG dispatcher, and a lighting / torch-burn timer.
...
The save system writes a single image of resident state — player position, plane, scene byte, inventory, active-object table, time-clock bytes, party stats, vehicle byte, scroll origin, and a few hundred bytes of flags. The image lives on disk as `SAVED.GAM` plus a companion file with the per-plane active-object seed.
...
A save is initiated from the Q-Quit command. The save logic does not care which mode is active — it freezes whatever the current resident state is. Combat is the one exception: the combat overlay never lets the player save mid-fight; the Q-Quit handler is gated on the scene byte being below the combat range.

## 12. Quit

Q-Quit is the one command that breaks the outer loop. The dispatcher routes Q to a save-then-exit handler; the handler writes the saved-game image, prints the goodbye message, restores the original video mode through the active driver, and calls the DOS terminate-with-return interrupt. The outer loop never sees the return.

Q-Quit is gated: the handler refuses if the scene byte is in the combat range or if the player is mid-prompt. Other than Q-Quit, there is no normal exit path. The outer loop's `return` from `main` is unreachable in normal play; the only way the program leaves the forever-loop is the DOS terminate from inside the quit handler.
...
**Save / load.** The intro overlay is the load path's entry; the Q-Quit handler is the save path's exit. The saved scene byte drives the outer loop's first dispatch on resume. See `save-load.md`.
...
- **Unreachable cleanup branches.** A handful of branches in the outer loop's cleanup pass — restore-video-mode on `main` return, the disk-swap busy-wait's retry counter — are unreachable in normal play because the only exit path is the DOS-terminate inside the quit handler. They exist as defensive code paths that a different boot/exit chain could reach.
...
- The DOS entry stub that establishes the stack and far-returns into `main` — `u5-decomp/functions/ULTIMA_EXE/0x81D0_boot_entry.md`.
...
- The cross-overlay trampoline stub mechanism, the loader's algorithm, and the buffer-sharing partition — `u5-decomp/functions/ULTIMA_EXE/0x75CC_overlay_loader.md`.
