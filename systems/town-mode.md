...
1. **Read a command.** The input pipeline blocks until a keystroke arrives, applies its translation rules (key-to-command, numpad-to-direction, queue handling), and returns a single byte. On its full-prompt arm that shared helper also drains the stats-panel refresh request - test, full repaint if set, clear - after the world-tick call and before the prompt newline and the key read; its quick-poll arm skips the test and leaves any request pending until a later prompt. Town's own paths never raise the request; they only consume it (`systems/stats-panel.md` Section 2.3).
...
4. **Per-turn epilogue.** When the dispatcher returns and the action consumed a turn, the loop snapshots the current hour, advances the time clock by one minute via the time spec's per-turn cleanup, runs the dawn/dusk gate pass if the new hour is `5` or `20`, runs the underfoot-effect handler described in Section 10, ticks down the curse/buff counter, and copies the party's current map coordinates into slot zero, so that both town walkers see the party at its post-move position on the same turn. It then runs the turn's movement work in a fixed order: three effect gates, the **town object walker** that moves the location's loose horse-family objects, and finally the NPC schedule processor with the current hour byte. `systems/npc-schedules.md` Section 5 owns the gates and the order; the arrest-cleanup result is **not** the only thing that can stop the processor. The cases are:

   - **Transport marker.** While the party's marker is one of the four values `0x12..0x15`, a stored parity bit flips each turn and skips *both* walkers on the turns it comes up set.
   - **Negate Time.** While that effect is active, *both* walkers are skipped outright — nothing in town moves.
   - **Quickness.** A second stored parity bit skips *both* walkers on alternate turns.
   - **A result code of two or greater,** which includes the explicit-T arrest-cleanup result. This one skips the schedule processor *only*, and it is tested after the object walker has already made its pass. The clock and earlier underfoot/status work still run, and arrest cleanup follows.

   *Corrected (R328):* an earlier revision of this step said the processor was called on every turn but the arrest-cleanup one — "the sole exception" — and did not mention the object walker at all.

   Town tests the three effect gates in the order given above; the overworld tests the same three in the opposite order, and the two orders are behaviourally distinct — see `systems/npc-schedules.md` Section 5 and `systems/encounters.md` Section 2.1.

   The underfoot-effect handler is called on every consumed turn and it re-reads the tile the party is standing on, so it is not gated on the party having moved; its last act is to run the shared party status/provision pass specified in `systems/time.md`.
5. **Render.** A full render runs when the turn's movement work reports a change, or when the visibility-dirty flag is set; otherwise the screen is left as-is and the loop reads the next command. The test has to cover **both** walkers, not just the schedule processor: the object walker runs on turns whose result code skips the processor, so a repaint conditioned on the processor's report alone leaves a moved object stale until some later turn repaints for another reason. (What is traced is that either walker can commit a step on a turn the other does not; whether the two signal through the same dirty flag was not separately traced.)
...
The arrest sequence itself branches on the current location before it prints anything. Inside Lord Blackthorn's Castle, and while the shared party-capability check reports that at least one member can act or is asleep, it plays the Blackthorn audience/capture cinematic and then re-runs town entry setup for the same location; `systems/blackthorn.md` owns that path. In Blackthorn's castle when nobody can act and nobody is asleep, the arrest helper returns silently without an audience or surrender prompt; the exploration loop's separate defeat check remains responsible for rescue. The earlier ordinary-surrender fallback for this palace case is withdrawn (R442). In every other location it prints the arrest challenge (`"Thou art under arrest!"`, a blank row) and asks whether the party will come quietly (`"Wilt thou come quietly?"`, a blank row, a `:` answer row that echoes `Yes` or `No!`; the surrender line is `The guard strikes thee unconscious!` - *literals added 2026-09-06, issue #206*): surrendering prints the knockout and awakening lines, fades, moves the party to the Yew jail scene at `(25, 4, 0)`, marks the view dirty, advances time in twenty-minute cleanup calls until the hour reaches 08:00, clears the jail-scene latch, and returns as a consumed turn. Refusing prints the guards' challenge, triggers the alarm sweep, and consumes the turn. Monster-class or attack outcomes can route through the NPC death flow, which marks the scene mask, clears the live slot, reloads the floor, and re-runs the Shadowlord install pass of Section 13 (normally a no-op, for the reason given above).
