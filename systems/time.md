...
- **Movement.** The baseline is one minute indoors and two outdoors. An accepted outdoor step also charges two or four minutes for difficult destination terrain, giving ordinary totals of four or six minutes; `systems/movement.md` Section 8.1 owns the full tile table and extra outdoor object-update calls. Horse and carpet travel retain these terrain charges. No vehicle changes the baseline mode increment; the `Q` half-increment applies only while Quickness from Section 4 is running, and the `T` suppression only while Negate Time is. Earlier wording attributing those effects to water transport/carpet travel remains withdrawn. The additional unqualified claim that outdoor moves cost two minutes is withdrawn for difficult terrain (`RETRACTIONS.md` R424).
