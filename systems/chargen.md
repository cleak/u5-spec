...
`INIT.OOL` is similarly read into a scratch buffer late in the flow, immediately before the new save is written. The relationship between the seed and the working file is asymmetric: the engine reads `INIT.GAM` and `INIT.OOL` whenever a fresh game starts, but writes only `SAVED.GAM` and `SAVED.OOL`. Reading the working file at chargen time would be wrong — the player might already have saved a previous game in the working slot, and starting a new one must not depend on that slot's contents. The working files must nevertheless already **exist**: the resident write wrapper opens the named file as an existing file, prompting for the right disk until it succeeds, before it truncates and rewrites it (`systems/save-load.md` Section 7.2), so a game directory without `SAVED.GAM` or `SAVED.OOL` leaves chargen waiting on a blank screen after the closing narrative instead of producing a save. *(Added 2026-09-05; observed live.)*
...
The avatar's class field is **not written by chargen**. The class byte stays at whatever `INIT.GAM` shipped — the ASCII letter `A`, denoting "Avatar". Despite the eight-virtues-eight-classes parallel from Ultima IV, Ultima V's chargen does not pick a Fighter / Bard / Mage / Druid / Tinker / Paladin / Ranger / Shepherd class for the avatar based on the questionnaire winner. The avatar is always class Avatar. The class letters that *do* appear in the save file belong to the companion records — all preset by `INIT.GAM` and not modified by chargen.

Only **four** distinct class letters occur anywhere in the shipped seed roster: `A` for the Avatar's slot and `M`, `B` and `F` for the companions. Iolo, Julia, Gwenno, Toshi and Saduj carry `B`; Mariah, Jaana and Johne carry `M`; Shamino, Geoffrey, Dupre, Katrina, Sentri, Gorn and Maxwell carry `F`. The letters `D`, `T`, `P`, `R` and `S` are legal in the save format (`formats/saved-gam.md` section 3.1) and are handled by the class-indexed tables in `systems/combat.md` and `systems/endgame.md`, but no shipped record uses them. Earlier revisions of this paragraph said "Shamino is a Ranger"; the byte in his slot is `F`, and that claim is **withdrawn**. The Ultima IV virtue-to-class pairing in `systems/karma.md` section 9 is thematic inheritance from the predecessor game, not a description of these bytes.
...
The observed transfer path renders a two-panel comparison preview of the single imported character, lets the player confirm or replace name/gender, and ends either by abandoning the transfer before it starts or by committing. The abort is narrower than chargen's: `Esc` at the drive-selection prompt is the only key that leaves the path, and it happens before any predecessor data has been read, so once the comparison stages begin there is no cancel (`systems/u4-transfer.md` sections 6.4 and 8). A source-validation failure is the other way out. Either way nothing is written; commit writes the standard Ultima V save pair and then returns through the intro flow, where Journey Onward performs the normal load.
