...
The on-disk schedule is read-only: a save game does not embed copies of `TOWNE.NPC` and friends. **Everything else does persist.** The schedule table loaded from that file, the per-NPC runtime block, the move queues, the queue read pointers, the type array and the stuck counters all live inside the save image, in the band immediately after the active-object table (`formats/saved-gam.md` Section 12), and are flushed and restored with it verbatim.

And the load path does not re-run the initialisation. A town-family scene reached by loading a save enters its setup pass in the preserving entry mode, which skips the `.NPC` roster load, the runtime-state initialisation from the hour, and the waypoint reseat alike (`systems/active-objects.md` Section 10). **Saving inside a town freezes each NPC at its exact position, with its queued path and pursuit target intact, and loading resumes from there.** The first scheduler tick after the load continues the route it was on; it does not recompute a waypoint from the saved hour and does not snap anyone anywhere.

*Corrected (issue #184).* This section previously said the runtime state "is *not* explicitly persisted as a chunk", that on save-and-load "the engine re-loads the schedule sub-block from the appropriate `.NPC` file, runs the same initialisation pass that runs on first entry (Section 4), and recomputes the active waypoint from the saved hour byte", and that "saving inside town does not freeze NPCs at their exact position; it resets them to 'currently scheduled location'". All of that is withdrawn. An implementation built on it not only re-snaps NPCs it should not touch — if it also declines to persist the band, it resumes a **completely empty** location, because the per-tick walker skips every roster slot whose type byte is zero and the type array was never reloaded. See `RETRACTIONS.md` row R341.

The initialisation pass of Section 4 remains exactly right for a *fresh* entry, which is the mode every non-load path uses.
