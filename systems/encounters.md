...
- **Visibility.** Off-screen monsters are pruned from the active-object table by the overworld per-turn epilogue (normative contract: `systems/active-objects.md` Section 8.1 - a per-axis window test, not a radius) but stay alive conceptually — the engine's "thirty-two-cell sliding window" means the player's far-away wanderings are not tracking specific monsters' positions.
