...
| 5      | In Zu                     | Sleep               | Directed cone sleep gated by shared resistance.            |
...
| 8      | In Vas Grav Corp          | Death Wind          | Directed instant-kill cone gated by shared resistance.     |
...
Good, Poisoned, or Sleeping member enters defeat/rescue; neither can dispatch
M-Mix. Because Mix changes no status before its own scan, every reachable wrong
mix targets the first Good-or-Poisoned in-party member. The shared word used for
that handoff is transient focus/direction state, not a world map origin, and the
scan does not change the party's live position. *Corrected:* an earlier revision
said the no-Good-or-Poisoned stale target was reachable, probably held a map
coordinate, and might disturb world position. All three claims are withdrawn.
Under forced direct invocation the unchanged prior word can still make Acid
index outside the roster; `systems/traps.md` § 4 distinguishes that memory-unsafe
ABI edge from reachable play and gives the safe implementation rule.
...
placed. Actor contact with arena fields is handled later by the common
post-dispatch hook described in `combat.md`. The round walker passes the
descriptor slot whose dispatch just returned, and that actor is the contact
target. The marker scan skips only that descriptor's linked renderer-facing
active-object record; it does not skip the current actor as target. A separate
Poison, Sleep, or Fire marker on the same coordinate therefore applies to that
actor, whether the completed dispatch belonged to a player or AI slot and
whether the completed action changed coordinates.

Combat fields are materialized as active-object markers in the temporary combat
table. Contact is non-consuming: the hook applies the mapped result without
clearing, aging, or rewriting the marker record. Poison Field first rejects a
target whose linked active-object tile/class byte is at least `0x80`. An accepted
Good party member becomes Poisoned without a random draw; another accepted
target takes one inclusive `0..20` raw-damage draw, with no defense draw and no
caster experience credit. Sleep Field skips dead party members; otherwise it
sets party-member status to asleep, or sets descriptor byte 2 bit `0x08` for a
non-party target and steadies its linked renderer animation. It does not seed a
separate combat sleep duration counter. Fire Field consumes one inclusive
`0..10` raw-damage draw after marker selection and makes no defense draw. Energy
Field is a blocking marker and has no result arm in this contact hook. Field
markers have no countdown or implicit removal path and persist until combat
exit restores the pre-combat active-object table.
...
- **Field status-helper edges.** The placement-time coordinate lookup and the later contact hook have different targets and skip rules. Placement accepts the first selected-coordinate descriptor with `0x80` or `0x40` set, rejects descriptors marked by `0x20` or `0x04`, and rejects linked active-object tile byte `0xF4`; that result does not choose the later contact target. The common post-dispatch hook instead targets the current actor descriptor and skips only its linked renderer record while looking for a separate colocated marker. Poison, Sleep, and Fire can therefore affect the current actor after either player or AI dispatch. Poison's Good-party status arm consumes no randomness; its damage fallback rolls raw 0..20 with no defense draw. Fire rolls raw 0..10 with no defense draw. Energy blocks movement and has no contact-result arm. The hook does not consume the matched marker. Combat marker placement requires confirmed in-arena impact resolution, but no Fire/Sleep/Energy random acceptance gate exists for materialization. Field markers persist until combat exit restores the pre-combat active-object table; no traced placement, contact, redraw, generic active-object tick, or monster death/record-clear path decrements or removes them earlier.
...
- The shared spell-resistance rating sources, signed score formula, equality
  behavior, caller census, and the distinct Tremor/Poison Wind target-only gate
  are derived from private analysis in
